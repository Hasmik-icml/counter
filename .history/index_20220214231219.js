...
document.queryCommandValue(".fibonachiButtons").addEventListener("click",function(){
   let next = fibonachiNumbers
  
   document.querySelector(".resultFibonachi").innerHTML =  fib(next);
})
...
function fib(next){
    let nextFib; 
    while(true){
        nextFib = fibonachiNumbers[i] + fibonachiNumbers[i - 1];
       next++;
    }
    return nextFib;
}